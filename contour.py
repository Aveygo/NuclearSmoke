...
class ContourFinder:
    """
    Given a Fire object, class call returns 3 contours for 0.5mSv, 3mSv, and 6mSv.

    Increasing the resolution results in more refined contours at the cost of compute.
    This will result in small vs large fires having different grid sizes.
    """
    def __init__(self, fire:Fire, resolution:int=200):
        self.fire = fire
        self.resolution = resolution

    def smoothing(self, polygon, sigma=1):
        """
        Apply gaussian smoothing to contours as cv2 results tend to
        follow pixel edges
        """
    ...
    def dose_calc(self, x, y, full_calc=False):
        ...
        Returns results in miles. I'm sorry.
        ...
        # Compute the expected roentgen for a given x/y array
        X, Y = np.meshgrid(x, y)
        w = WSEG10(
    ...
    def contours_to_map(self, c, min_x, min_y, max_x, max_y, lat, long, direction, h_scale, v_scale):
        """
        Given cv2 contours, adjust scaling, location, and rotation to match destination target.
        """
        c = [i[0] for i in c]
    ...
    def calc_contour(self, gray, thresh, min_x, min_y, max_x, max_y, h_scale, v_scale):
        """
        Convert dose image into contours used in final result
        """
    ...
    def __call__(self):
        """
        Main method to convert self.fire into final contours
        """
        min_x, min_y, max_x, max_y = find_bounds(self.dose_calc)
        v_scale = (abs(max_y) + abs(min_y)) / self.resolution # set image width & height to self.resolution
        h_scale = (abs(max_x) + abs(min_x)) / self.resolution # 
        ...
        gray = ((z/5000)*255).clip(0, 255).astype(np.uint8) # Possible to adjust scaling to effect contrast 

        c10  = self.calc_contour(gray, 10,  min_x, min_y, max_x, max_y, h_scale, v_scale)
        c80  = self.calc_contour(gray, 80,  min_x, min_y, max_x, max_y, h_scale, v_scale)
        c250 = self.calc_contour(gray, 250, min_x, min_y, max_x, max_y, h_scale, v_scale)
...
if __name__ == "__main__":
    ...
    # Skip the __call__ method to manually calculate dose image. 
